"""Offer-compliance policy, ported from v0 with unchanged semantics.

Port of v0 ``proxyloop_contracts/offer_policy.py`` (``offer_compliance_violations``
and ``unsupported_applied_changes``). ``offer_violations`` is v0
``offer_compliance_violations`` plus the bounded applied-change check that the
v0 negotiation catalogue appended (``negotiation_catalog.offer_violations``).
``tests/fixtures/v0/offer_policy.json`` pins the v0 behaviour.
"""
...
def is_supported_applied_change(change: str) -> bool:
    """Return whether one Provider change token is inside the bounded set."""

    if change in SUPPORTED_APPLIED_CHANGES:
        return True
    return change.startswith(REMOVE_ADD_ON_PREFIX) and len(change) > len(
        REMOVE_ADD_ON_PREFIX
    )


def unsupported_applied_changes(changes: tuple[str, ...]) -> tuple[str, ...]:
    """Return the unsupported change tokens in first-seen order, deduplicated."""

    return tuple(
        dict.fromkeys(
            change for change in changes if not is_supported_applied_change(change)
        )
    )
...
def offer_violations(
    ...
    # Credits arrive as bounded change tokens, not as a separate field. Only
    # the catalogued promotion credit may reduce the total; unknown
    # credit-like tokens do not bypass fee consistency.
    ...
    if unsupported_applied_changes(terms.applied_changes):
        violations.append(UNSUPPORTED_APPLIED_CHANGE)
...
def _require_tokens(values: tuple[str, ...], *, name: str) -> None:
    if not isinstance(values, tuple):  # pyright: ignore[reportUnnecessaryIsInstance]
        raise ValueError(f"{name} must be a tuple")
    if len(values) != len(set(values)):
        raise ValueError(f"{name} cannot contain duplicates")
    if any(not isinstance(value, str) or not value.strip() for value in values):  # pyright: ignore[reportUnnecessaryIsInstance]
