...
def record(model: str = MODEL) -> dict[str, Any]:
...
def test_decimal_split_across_chunks_is_not_a_sentence_end():
    ...
    # A terminator at the end of the text counts only once finished.
    assert r["ttfs_s"] is None
...
def measured(
    ttft: float | None, ttfs: float | None = None, error: str | None = None
) -> dict[str, Any]:
...
def test_summary_percentiles_and_failures():
    ...
    # numpy.percentile(1..20, 95), linear
    assert s["ttft_p95_s"] == pytest.approx(19.05)
...
def test_a_request_without_a_sentence_end_fails_ttfs_but_is_counted():
    ...
    assert (s["n"], s["failed"], s["ttft_n"], s["ttfs_n"], s["ttfs_complete"]) == (
        2,
        0,
        2,
        1,
...
def block(ttft: float, ttfs: float | None) -> dict[str, Any]:
...
def test_derived_rows():
    ...
    lat = {
        f"{base}@c1": block(0.30, 0.50),
        f"{zero}@c1": block(0.32, 0.55),
        f"{base}@c4": block(0.40, 0.70),
        f"{zero}@c4": block(0.40, None),
        f"{base}@warmup": block(9, 9),
    }
...
def raw_report(keyless: dict[str, int]) -> dict[str, Any]:
    ok = {"complete": True, "max_abs_diff": 0.0, "mean_abs_diff": 0.0}
    summary = {"failed": 0, "ttfs_complete": True, "ttft_p50_s": 0.3, "ttfs_p50_s": 0.5}
    lat = {
        f"{m}@{b}": {"summary": summary}
        for m in (config.SERVED_NAME, config.ZERO_LORA_NAME)
        for b in ("warmup", "c1", "c4")
    }
    return {
        "keyless_status": keyless,
        "attest": {"runtime": {"lora_rung": "all"}},
        "tokenize": {"all_equal": True},
        "latency": lat,
...
def test_keyless_401_is_composed_from_both_protected_paths(
    statuses: dict[str, int], expected: bool
) -> None:
...
def test_evaluation_error_is_recorded_and_the_raw_run_kept(
    ...
    monkeypatch.setattr(probe, "base_url", base_url)
    monkeypatch.setattr(probe, "measure", measure)
    monkeypatch.setenv("PROXYLOOP_VLLM_API_KEY", "k3y")
    # No rung_ok -> still evaluates.
    (tmp_path / "ladder.json").write_text(json.dumps({"summary": {}}))
    ...
    assert (
        probe.main(["--out", str(out), "--ladder", str(tmp_path / "ladder.json")]) == 1
    )
