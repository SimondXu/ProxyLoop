...
5. **World structured-call policy (root decision, 2026-09-26).**
   - A schema-invalid world structured call (the Ear, or any forced world tool call) is recorded as a counted error event, the same kind of metric as `fidelity_fallback`.
   - The world may regenerate at most 2 times, the bound ARCHITECTURE §10 gives the Mouth. Each attempt is its own `llm.call` record.
   - If the call is still invalid, or a world call hits its wall-clock timeout, the episode ends with an error. The output is never coerced, defaulted or silently skipped.
   - The per-episode count of invalid and regenerated world calls is reported next to the Ear metrics.
   - Reason: this bounds noise in the simulator, not in the model under test. It does not touch Fast parsing (AGENTS rule 12 is unaffected). Without it, world noise alone could trip the EVAL §9 integrity gate (> 5 % errored episodes).
   - S0-SYS-05 implements and tests this policy.
6. **Latency bound.** S0-SYS-05 bounds each world call with a wall-clock timeout plus an explicit `reasoning_effort` on every `WorldModels` role, not with `max_tokens` (see Risks). A timeout is an error event under Decision 5, never a silent default.
...
- `docs/decisions/data/teamrouter-probe-forced.json` (`uv run --no-project scripts/spikes/relay_probe.py --env <abs path>/.env.local --env-format dotenv --task S0-ROOT-08 --part forced --models gemini-3.8-flash --forced-n 10 --forced-max-tokens 512 --out docs/decisions/data/teamrouter-probe-forced.json`):
...
- `docs/decisions/data/teamrouter-probe-main.json` (`uv run --no-project scripts/spikes/relay_probe.py --env <abs path>/.env.local --env-format dotenv --task S0-ROOT-08 --models gemini-3.8-flash --ttft-n 10 --stream-max-tokens 512 --out docs/decisions/data/teamrouter-probe-main.json`):
...
- **Risks (for S0-SYS-05 to implement and test):**
  - **Schema-invalid Ear output.** `schema_valid_rate` is below 1 in this run: a forced call can omit required fields (`calls[].schema_errors` records `missing:amount_usd`). Decision 5 governs this. Schema-valid is also not correct: the numeric cross-check in ARCHITECTURE §10 still applies (compare `calls[].args` with the utterances).
  - **Reasoning by default.** `reasoning_tokens_p50` is well above zero, and the probe ran with the provider's default effort. Each role's `ModelRef.reasoning_effort` must be set explicitly, as the contract requires. S0-SYS-05 picks the value with a probe; this ADR does not assume one.
  - **`max_tokens` does not cap reasoning on this route.** In `teamrouter-probe-forced.json`, `results.gemini-3.8-flash.calls[6].usage.completion_tokens` is far above `forced_max_tokens`, and `calls[6].latency_s` is the latency outlier of the run. `max_tokens` therefore bounds neither cost nor latency here; Decision 6 bounds latency instead.
