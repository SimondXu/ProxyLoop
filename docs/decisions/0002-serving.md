...
- **LoRA ladder** (`scripts/mod/lora_ladder.py`, its own Modal app, one offline vLLM engine with the served
  ...
  **applied** only when its probe loads and moves the mean |Δ prompt_logprob| above 1e-3 nats. A rung is OK
  ...
  then exits non-zero. A stale `data/vllm-lora-ladder.json` from an earlier run is renamed to
  `vllm-lora-ladder.superseded-<UTC time>.json` (kept, never deleted), so the `serve-up` order guard stays
  closed. Once the Modal function has computed the result, it saves it on the `proxyloop-adapters` volume
  (`results/lora-ladder-<UTC time>.json`, named in `volume_copy`) before returning it. If the local process
