...
| Consumer / Provider / Channel Event | Built for Consumer events (the Web and API), the fictional Provider's events, and the synthetic `local_mailbox` channel; no real Provider or channel. |
| Fast Turn Decision | Built as a canonical contract (`FastTurnDecision`, `runtime/packages/contracts/src/proxyloop_contracts/contracts.py:738`). |
| Slow Work Result / Strategy Packet | Built as canonical contracts (`SlowWorkResult`, `contracts.py:1464`; `StrategyPacket`, `contracts.py:302`). |
...
- Intake (PR-12) is a typed card, not a step-by-step wizard. The consumer's first message goes once to the stateless `POST /intake/proposals`, and the returned Intake Proposal fills the Draft Task Brief card: current monthly total, target monthly total, mobile hotspot required, device financing change forbidden. A value the proposal read is labelled "Read from your message", never "Confirmed", until the consumer supplies it again or chooses Create. A fact the proposal could not read shows its clarification ("Missing", "Needs clarification · …", or the value plus the rule it breaks), and only those facts are asked for; the consumer answers in the composer or uses Edit, with the same strict local parsing, the $72 rules, and the $999,999.99 cap. After an edit the other amount keeps its rule code until its own rule passes, and any amount the local rules still reject is prompted, so Create is never disabled without a prompt. Only the explicit "Create fictional Case" click sends the four typed facts to `POST /cases`, so a Case is created only from consumer-confirmed typed facts. The card opens if the proposal read at least one value, if the text is a mobile-bill request, or if the proposal has an amount clarification (other than `missing` or `unsupported_currency`) and the text holds a bill or payment cue from a closed list (`bill`, `pay`, `monthly`, `/mo`, `carrier`, `phone`, `data`, `hotspot`, `financing`, …; bare `plan` is not one). Otherwise the scope reply is shown. Any failure of the proposal request shows "I couldn't read that message right now. Nothing was created." The request text is kept only in the in-memory transcript, never in the browser envelope.
...
- `POST /intake/proposals` (PR-12) is stateless. It takes `{"text"}` (1–2000 characters) and returns an Intake Proposal: the four `CreateCaseRequest` keys (Money or `null`, `true` or `null`), at most one clarification per key from a closed set (`missing`, `ambiguous`, `invalid_amount`, `unsupported_currency`, `below_fixed_offer`, `target_not_below_current`), and the rule-set name `intake-parser-v1`. The parser (`proxyloop_api/intake.py`) is pure, deterministic, and model-free in every adapter mode. Its principle is that any uncertainty becomes a clarification, never a guessed value. It reads `$N` / `N USD` / `N dollars` amounts (ASCII digits only, at most $999,999.99) and assigns each to the current bill or the target by closed cue words read from the clause before it: a target cue beats a weak current cue ("I only want to pay $75"), and a target cue that meets a strong current cue ("my bill is", "I pay", "currently") leaves the amount without a role. A change amount ("save $20", "$10 off", "by $12"), a range ("between $75 and $80", "$70 to $80"), an amount after a price-history verb ("went up to $92", "jumped from $85 to $110"), an amount in a question (unless the question asks to lower the bill, as in "Can you lower my phone bill from $92 to $75?"), or any amount with no cue has no role, and then both amounts are `ambiguous`; more than 8 amounts are not read one by one. The work is bounded: text longer than 4000 characters after NFKC is not read (every field `missing`), the end-anchored patterns see only a 48-character tail, and a timing test holds adversarial 2000-character input under 250 ms. It reads keep and never-change phrases for the two supported features; a negation (including `n't` contractions), a hedge, a question, or a later clause that negates or changes something without naming a feature makes the feature `ambiguous`; a retraction ("No.", "never mind", "scratch that", "just kidding", …) makes every named feature `ambiguous`. Its amount rules and cap equal `CreateCaseRequest`'s, pinned by a parity test; `CreateCaseRequest` gained the same $999,999.99 cap on the current bill (the target must already be lower). The route creates no Case and calls no Runtime, repository, Temporal, or model. The text is never logged, echoed, or persisted: the response holds only Money, booleans, and codes; the operation record has no body; a bad request gets the content-free 422. The proposal is an API model, not a canonical contract, and no `CaseCommandType` was added. English only, no number words or bare numbers.
...
The Fast/Slow split is sequential, not concurrent: when the strategy has expired or is incompatible, the Runtime runs the Slow refresh before the Fast call on the same turn, so the line waits for Slow. `case_runtime/turn_split.py` measures the split per turn from the trace log and the final state (a turn is an applied `provider_offer`, `consumer_message`, or `provider_message`; traces join by input cursor, the last Fast trace at a cursor is the delivered attempt, and a repeated `create_case` counts as an unapplied attempt). Since PR-14 a Slow call is followed by its Judge call and, after a `revise`, by one Slow retry; the split parses these as one group per attempt and refuses a Judge trace that follows no admitted Slow call. Judge calls and Slow retries are counted apart (`judge_calls`, `slow_retry`, `judge_calls_by_result`, `slow_retry_counts`); every Fast and Slow count and share keeps its version 1 meaning and value, and no verdict is reported. The report is `fast-slow-split-v2`. A Slow call after a `revise` is read as the retry unless a Judge call follows it (a retry is never judged); a new attempt's Slow call that is rejected right after an unretried `revise` is indistinguishable from a rejected retry and is counted as one (a known limit). The local Fast split reports observed through PR-9b's gateway (`fast-slow-split-distilled.json`, `fast-slow-split-untuned.json`) predate the Judge: they are observed artifacts that only the local model can regenerate, stay at `fast-slow-split-local-v1`, carry no Judge calls, and are not rewritten by PR-14. The integrity check still accepts them, because their Fast/Slow turn structure equals the v2 scripted replay's (on the default Slow the Judge adds no Slow or Fast call). A local report written now is `fast-slow-split-local-v2`: it carries the Judge's calls, apart, and the local mode refuses a Judge trace that is not the scripted Judge's. Log order is call order only within one process (the Case lane and the direct lock); across processes a concurrent losing attempt at the same cursor can be taken for the delivered one until traces carry a command id. The aggregates count applied turns and calls (`gate_fallback_rate` is gate fallbacks over applied Fast turns); unapplied attempts are reported apart. `make fast-slow-split-check` (in `make test`) replays the committed `data/evaluation/fast-slow-split-scripted.json`. That report describes routing structure only, not model quality or latency.

#### Local opt-in Fast backend (PR-9a)

The Fast Backend is the one Fast adapter a Runtime process uses. `PROXYLOOP_FAST_BACKEND` selects it at start: `scripted` (the default, `ScriptedDialogueFastAdapter`), `distilled` (the Phase 03C adapter, a Local Opt-in Candidate), or `untuned` (the untuned base, the baseline). A local backend is allowed only with `PROXYLOOP_RUNTIME_MODE=scripted` (Slow stays scripted, decision 17) in direct orchestration or under Temporal (PR-11, below). `proxyloop_local_fast.fast_adapter_from_environment` is the one parse. The process refuses to start on an unknown value, a gateway URL that is not an `http://` loopback origin (`127.0.0.1`, `::1`, `localhost`; no credential, path, or query), a timeout outside [0.1, 25] seconds (`PROXYLOOP_FAST_TIMEOUT_S`, default 25, the cap; the cap stays under the 30 s Temporal activity and Next proxy limits), a gateway that does not answer `/v1/identity`, or one that serves another backend. Rollback is setting the variable back and restarting; nothing switches backends automatically. `adapter_mode` becomes `local_distilled_candidate` or `local_untuned_baseline`; an adapter that names any other backend label is refused at Runtime construction; the readiness and operation payload keys do not change. The Web restores a saved Case after a page reload only when readiness reports `orchestration_mode=temporal`, `storage_mode=postgres`, and an `adapter_mode` of `scripted`, `local_distilled_candidate`, or `local_untuned_baseline`; direct orchestration (not durable) and every other `adapter_mode`, including the hosted `model`, get no recovery claim. Outside this allow-list, `adapter_mode` is opaque to the Web.
...
Recorded local limits: a Fast call holds the direct-mode app lock (B2-8) for up to the timeout, 25 s by default, blocking other direct commands. PR-9b measured the distilled backend on one Apple-silicon machine, sequentially, under uncontrolled load (not p95 or capacity). Through the product path, 64 of 200 generations ran over 25 s by `generation_ms` (65 by `wall_ms`), roughly a third. At the 25 s default such a call ends as `fast_adapter_timeout` and delivers the fallback line. For context, the trained prompt path had 15/240 over 25 s, and the local split runs saw 0 of 16 calls time out (21.0–24.1 s); MLX cannot cancel a generation, so a call after a timeout can get `busy`; the loopback gateway has no authentication, which suits a local opt-in demo and is not a deployment design. CI runs only an in-test fake gateway; no latency, p95, capacity, or model-quality claim is made here.

Measured through the product path (PR-9b, one Apple-silicon machine, sequential, other work running on it; `ml/serving/README.md`), the result is negative and is recorded as such. Of the same 240 held-out rows rendered by `fast_public_observation`, the distilled backend delivers 0 lines. 40 rows (refusal-transfer, no offer) are refused before the model. All 200 outputs that reach `fast-gate-v1` are withheld on the dialogue-act and undisclosed-number rules. The untuned baseline delivers 8/240. Act agreement with the true oracle falls from 0.983 on the trained path to 157/240 = 0.654 (untuned 97/240). About half of that drop is D4, the product observation carrying no applied changes: unsupported-action, −40. The other half is refusal-transfer, whose 40 rows have no offer and are refused before the model (−40). Required-feature-loss gains one row. On promotion-credit the model keeps its act although D4 changes the input. Distilled product-path generation took p50 23.9 s and max 32.5 s (timeouts above). The committed local split reports (`data/evaluation/fast-slow-split-{distilled,untuned}.json`) keep the scripted turn structure. On both backends every Fast turn (8 per backend) delivered the fallback line after a gate reject, and none ended in a timeout or `busy` failure. So with the local backend a person sees the fixed fallback line, not model text.

#### Local Fast backend under Temporal (PR-11)

The Temporal worker selects its Fast adapter from the same `PROXYLOOP_FAST_BACKEND`, `PROXYLOOP_FAST_GATEWAY_URL`, and `PROXYLOOP_FAST_TIMEOUT_S` through the same `fast_adapter_from_environment` (`activities.activity_adapter_from_environment`), so it refuses to start exactly where the API does, and it does so before it opens the database. The API no longer refuses a local backend under Temporal: it reads the same variables, probes the same gateway at start, and labels `adapter_mode`, but it never calls Fast in Temporal mode (every command is a Workflow Update). The API cannot see the worker's selection; the 07A launcher gives both processes one environment, the worker logs its backend label once at start (`worker Fast backend: <label>`, the label only), and the per-call Fast traces the worker writes are the authority for what served a turn. A worker and API started by hand with different values disagree silently: with the worker on `distilled` and the API on `scripted`, `/health` and the operation records say `scripted` while the traces name the local model (the label under-claims); the reverse over-claims a local backend that no call used. Even when they agree, `adapter_mode` in the operation records and `/health` describes the backend of the Case commands only, not the channel path, whose Fast stays scripted (below). The API's `runtime_from_environment` opens its database (PostgreSQL bootstrap) before it probes the gateway, so an API refused for an absent gateway has already connected to the database once; the worker probes first.

No Workflow command, timer, patch, activity option, or retry policy changed, so no replay fixture is needed. The budget: the Fast call is bounded by its timeout (at most 25 s, connect through last byte), which leaves at least 5 s of the 30 s activity start-to-close for scripted Slow and storage, and the Web's Next rewrite proxy also times out at 30 s. A captured Fast failure (timeout, busy, unavailable, protocol error, invalid output, identity mismatch, unrenderable input) applies the command with the fallback line and a `FAILED` trace, so the activity completes and Temporal has nothing to retry. A validation reject is `model_path`, which is non-retryable. The single-flight gateway answers a concurrent call from another Case with `busy` at once, and that turn gets the fallback.

Channel commands (`ingest_channel_event`, `record_channel_delivery`) keep the scripted Fast. With a local backend the worker applies them on a second `ThinAgentRuntime` over the same repository with the scripted default, so the outbound body stays `BOUNDED_FAST_STATUS_TEXT`, the equality check stays, channel ingest adds no Assistant Message, and the model is never called on the channel path (PR-8 I7). Model text on an outbound channel would be a send under `SEND_MESSAGE` authority and remains undecided. The two runtimes have separate in-process Case lanes, as two worker processes do; per-Case commands are serialized by the Workflow, and PostgreSQL compare-and-swap admits one commit.

Recorded limits under Temporal: a second command for the same Case waits behind the first on the Workflow lock, so two queued 25 s Fast calls can push the second Web request past the 30 s proxy while its command still applies (the Web's idempotency key makes a retry deduplicate). A slow database plus a 25 s Fast call can still exceed the activity start-to-close. A retry of that activity returns the committed receipt without a model call when the first attempt committed first. A retry that races the still-running first attempt in the same worker waits on the per-Case lock, and is stopped before its Fast call only by a conflict raised inside the event path: with `expected_revision` sent, the revision check (resolved to the first attempt's receipt, `deduplicated`); without it, only incidentally, because today's scripted first consumer turn opens an approval and the retry fails "awaiting approval". `_append_event_serialized` does not re-check the command's receipt after taking the lock; PR-13, which owns `runtime.py`, adds that re-check. A retry that lands on another worker process has no shared lock and can make its own model call before PostgreSQL rejects the second commit. `FAST_BACKEND=distilled|untuned make portfolio-demo` expects PR-9b's gateway to be running already and refuses before starting anything if it is absent or serves another backend ([portfolio-demo.md](portfolio-demo.md)).
...
The Runtime keeps the capability proposal of the last admitted Slow result as the Case's standing proposal (`CaseRuntimeState.standing_proposal`, persisted as an optional field of the `storage_version` 3 envelope; it appears in no snapshot, view, API body, or trace). Each admitted Slow result replaces it, with none when the result proposes nothing or the Case already holds an approval. A Consumer event opens an Approval Request only when the standing proposal is admissible at the event time (`standing_proposal_offer`: the manifest's accept-offer capability, nothing expired, exactly one `offer_id` naming a snapshot offer) and `offer_compliance_violations_for_case` finds that offer compliant; the Runtime still compiles the intent, the approval, and the executor's proposal itself, and the approval consumes the standing proposal. Otherwise the command applies, the assistant line is delivered, and no approval opens. The scripted default Slow (`ScriptedProposingSlowAdapter`) proposes the accept on every call while the offer is live, so the scripted Runtime opens exactly the approvals it opened before PR-13; a model Slow that proposes nothing opens none. No Slow re-consult is triggered when an approvable offer has no admissible standing proposal (deferred). Known limit, model Slow only (decision 17 keeps Slow scripted in every authorized flow): the OpenAI Slow compiler's proposals expire 5 minutes after the Slow call, so a Consumer who confirms later gets no approval until the next strategy refresh (30 minutes). In the worst case no approval is possible at all. A channel event at +31 minutes refreshes, its proposal expires at +36, and the next refresh at +61 minutes comes after the offer expired at +60. Approval state, executor dispatch, adapter configuration, and Evidence rules remain deterministic internal mechanisms rather than model "skills." The current runtime has one simulator capability family, so it does not introduce a parallel skills registry or a speculative internal capability catalog.

The Runtime's coordinator also has an advisory Judge (PR-14; `agent_core/judge.py`, passed only by `ThinAgentRuntime._coordinator()`, decision 7). It reviews every Slow result that validation and the A-3 check admitted, on create and on every refresh, before Fast and before deterministic policy; the ML evaluator's coordinator has none. Its verdict is `accept` or `revise` with closed reason codes and no text; there is no `block`. A binding `revise` retries a Slow adapter that implements `FeedbackReasoningSlowAdapter` once, on the same Slow Work Request, with the verdict passed in process: it is not a contract field (decision 20) and is never read from the trace log. The retry is admitted exactly like the first result and is final; a rejected retry, a typed `JudgeAdapterFailure`, a verdict for another request or result, or a return that is not a verdict (the last two are recorded as a `REJECTED` Judge trace) leaves the first admitted result, and a Slow adapter without the feedback protocol is not retried. Each Judge call is one `role=judge` trace and each retry one `role=slow` trace, appended with the run's other traces before the Runtime acts. The coordinator's outcome has no Judge field; the Judge's codes appear only in its `audits` and `traces`, and the Runtime never reads `audits` and only appends `traces` to the log (a source guard pins both), so it never reads a verdict. The Judge is scripted in every mode (decision 17; it is not part of `adapter_mode`): `ScriptedJudgeAdapter` revises (`judge_premature_give_up`) only a result that proposes nothing while an offer is live and the manifest can accept it, which the default `ScriptedProposingSlowAdapter` never returns, so the default Runtime's decisions and state are unchanged and only the trace log gains the Judge's calls. Known limits, unreachable while no product Slow implements the feedback protocol: an exception raised by the retry call propagates and loses that run's traces; and a retry that the coordinator admits but the Runtime cannot use (no strategy on create, or a strategy revision that does not advance) fails the command although the first result was usable.
...
The confirmation hash is the only content hash that completion verification uses: the completion verifier checks it against the Provider's confirmation (`evidence_hash_mismatch`), and a 1.1 `CompletionReceipt` recomputes it from its own confirmation fields as `confirmation_content_hash`, which a 1.1 `CaseContextSnapshot` requires to match the confirmation Evidence. A `simulator_transition` Evidence is an executor attestation minted in `prepare()`, before the Provider commit, so it names no Provider artifact; its hash is whatever its producer defines (the runtime hashes its own idempotency key, the ML evaluation runner hashes its simulator attempt) and must not be relied on. Separately, the PostgreSQL codec replays the deterministic simulator when it loads a Case and requires the stored quote, confirmation, and simulator-transition Evidence to equal the replayed values; that storage-integrity check ties stored rows to the runtime's current formulas, so changing any of them is a storage decision. The replayed offer uses the Case's own offer TTL: `ThinAgentRuntime(offer_ttl=...)` (default 1 hour; positive, whole seconds, at most `MAX_OFFER_TTL`, the 9-day Case deadline, one constant in the Provider module) is persisted as the optional `provider_offer_ttl_seconds` of the `storage_version` 3 envelope, bounded by the same constant on read, omitted for the default and refused if stored as the default or as an explicit null, so a default row is unchanged and a stored `expires_at` is still compared rather than trusted; the codec also refuses an offer that outlives the Case's capability manifest (R-6). This is integrity by replay, not authentication: a consistent forgery of every dependent field within the 9-day bound is accepted, the same class as the R-18 limit. `tests/integration/test_contract_semantics_limits.py` recomputes every hash of a completed runtime Case from this table.
...
What the flow above actually ran, by the end of Phase 07: steps 1 and 3–5 (Phases 01B, 02 and 03A1, including the untuned and frontier-reference baselines), and a teacher-distillation form of steps 8–11 in Phase 03C (generated scenarios, `claude-sonnet-5` teacher samples filtered by schema, oracle agreement and detectors, LoRA on the Fast output, evaluation on six held-out families). Step 2 is built as a gate but has ingested only the project's own synthetic source: the Phase 02 pipeline requires a provenance and license record on every row and rejects a missing or unapproved license, and the only licence it has admitted is `LicenseRef-ProxyLoop-Synthetic-1.0` (`harness/build/phase-02-data-factory.md`); no external dataset was ingested. Not done: the open-data SFT of steps 6–7 (the Phase 03B smoke that stood in for it stopped at `NO_GO_STOP_PHASE03B`), random human review in step 9 (none is recorded for Phase 03C), and step 12 (no model is promoted). Step 11 is only partly met by Phase 03C: one training seed, no paired Fast/Slow baselines (it evaluated the Fast output alone), and held out by scenario family only: there is no separate entity-held-out split, and no provider-held-out split (both provider configurations appear among the 240 held-out rows, 120 each). The details are in [ML evidence](ml-evidence.md).
...
- The Judge is advisory: it never authorizes, blocks, or edits a proposal, and its output never enters an evaluation metric or reaches the executor or an approval. `tests/contract/test_judge_boundary.py` keeps it out of `ml/`, `scripts/`, the policy, executor, admission, router, gate, storage and split modules.
