...
- Intake (PR-12) is a typed card, not a step-by-step wizard. The consumer's first message goes once to the stateless `POST /intake/proposals`, and the returned Intake Proposal fills the Draft Task Brief card: current monthly total, target monthly total, mobile hotspot required, device financing change forbidden. A fact the proposal could not read shows its clarification ("Missing", "Needs clarification · …", or the value plus the rule it breaks), and only those facts are asked for; the consumer answers in the composer or uses Edit, with the same strict local parsing and $72 rules as before. Only the explicit "Create fictional Case" click sends the four typed facts to `POST /cases`, so a Case is created only from consumer-confirmed typed facts. If the proposal read no fact and the text is not a mobile-bill request, the scope reply is shown instead of a card. The request text is kept only in the in-memory transcript, never in the browser envelope.
...
- `POST /intake/proposals` (PR-12) is stateless. It takes `{"text"}` (1–2000 characters) and returns an Intake Proposal: the four `CreateCaseRequest` keys (Money or `null`, `true` or `null`), at most one clarification per key from a closed set (`missing`, `ambiguous`, `invalid_amount`, `unsupported_currency`, `below_fixed_offer`, `target_not_below_current`), and the rule-set name `intake-parser-v1`. The parser (`proxyloop_api/intake.py`) is pure, deterministic, and model-free in every adapter mode. It reads `$N` / `N USD` / `N dollars` amounts, assigns each to the current bill or the target by closed cue words, and reads keep and never-change phrases for the two supported features; anything else becomes a clarification, never a guessed value. Its amount rules equal `CreateCaseRequest`'s, pinned by a parity test. The route creates no Case and calls no Runtime, repository, Temporal, or model. The text is never logged, echoed, or persisted: the response holds only Money, booleans, and codes; the operation record has no body; a bad request gets the content-free 422. The proposal is an API model, not a canonical contract, and no `CaseCommandType` was added. English only, no number words or bare numbers; "went up to $92" reads as a target, which the consumer corrects on the card.
