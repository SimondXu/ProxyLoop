...
- The Agent Status Bar (PR-10) is the first section of the right-hand context rail. `renderStatusBlock(payload)` in `apps/web/lib/status-block.ts` is the only place its text is built. It is a pure, clock-free rendering of the allow-listed browser projection: a "doing now" line, derived from the Case phase with the approval, execution, and completion overlays (the Runtime keeps `negotiating` while executing and after an expiry), then the phase, goal, constraints, current offer, approval with its expiry, execution, and completion. The completion row says verified only when the completion Evidence predicate passes. It needed no API or projection change. It does not read the `fast` echo, does not show channel delivery or the Fast gate verdict (neither is projected), and does not rank offers or compute compliance. It is a display for the person, not a model prompt. PR-8 invariant I12 is pinned by `tests/contract/test_status_block_boundary.py`: the disclosure gate imports only the standard library and `proxyloop_contracts`, and no source under `agent_core`, `openai_adapter`, or `ml/` names a status-block renderer. `apps/web/lib/status-block.test.ts` pins that only the conversation workspace imports the module. The rail is hidden below 1120 px, and so is the Status Bar.
