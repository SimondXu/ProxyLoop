...
| Local intake draft | Draft Task Brief with four missing/confirmed rows and Edit actions | Supply or edit one fact; create only when all are valid | Strict USD parsing (one `$` or `USD` amount, comma-grouped or plain digits, at most two decimals; a trailing sentence period is allowed, a decimal comma or third decimal is rejected) and fixed constraints stay local; no Runtime call occurs before explicit create |
