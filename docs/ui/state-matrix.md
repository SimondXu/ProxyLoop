...
| Restoring Case | Saved locator checks readiness, then reads the authoritative Case | Wait or reconnect | Only explicit Temporal/PostgreSQL/scripted readiness enables a durable-recovery claim; direct mode makes no such claim |
...
| Finalizing / pending execution | Approved terms remain visible with a Finalizing Progress artifact | Wait or reconnect | `pending_execution` is recoverable; no receipt is shown until a final GET verifies completion |
...
| Authoritative approval expired | Expired state with approval action removed | Start a new local demo | Browser time may disable approval and trigger a read, but only the Runtime labels `Expired` |
