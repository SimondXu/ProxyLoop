...
| M6 | Follow-up after PR-9b merges. | Recorded in the log as follow-ups for the merge: PR-9b's `_local_scenario` should check the Judge traces' `model` (as the scripted report does), and `LOCAL_SCHEMA_VERSION` must bump if the local reports are ever regenerated (they would then contain Judge calls). `docs/architecture.md` (split paragraph) now states that the local reports predate the Judge, carry no Judge calls, and are not rewritten by PR-14 (spec §7). |

## Verification after the fixes

See `harness/log/feat-pr14-judge-seam.md`, "Review fixes". The DB gates were
not rerun: `runtime.py` did not change (only the G1 test that counts its
source did).
