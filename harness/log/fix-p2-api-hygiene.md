...
- **R-2** `runtime/services/api/src/proxyloop_api/app.py`
  `handle_not_found` / `handle_conflict`: 404 → `{"detail": "case not
  found"}`, 409 → `{"detail": <category>}` (`stale_cas` | `case_conflict`,
  unchanged `_conflict_category`), the same string shape the Temporal branch
  returns. Status codes and operation-record categories unchanged. New
...
- `test_phase_04a_agent_runtime.py`: `case is awaiting approval`,
  ...
  `case snapshot revision is stale` → `stale_cas`.
...
- Not run (root to schedule serially): `make postgres-check`,
