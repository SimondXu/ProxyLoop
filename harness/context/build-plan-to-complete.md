...
| PR-15 | 5 | **Dropped (decision 21).** Narrow contracts 1.2 (decision 19: R-11b, B1-9b); both stay recorded limits | `contracts.py`, contract fixtures | PR-10, PR-13 | **yes** | `make preflight` with every committed `*-check` byte-identical; DB lane |
...
| PR-17 | 6 | Reports: `docs/ml-evidence.md`, architecture reconciliation (A-7f), observed-vs-proposed README, limitations and negative results, cost (relay ≈ USD 146 actual usage for all Phase 03C Stage 1b/1c runs, of which USD 121.59 is the accounted estimate for the v6 full generation run, per `harness/log/phase-03c-stage1c-full-generation.md`; the 146 covers 03C only, and the 03A1 relay costs are listed separately; Modal USD 22.25; corrected 2026-09-25, see `docs/limitations.md` "Cost"), fresh-clone reproduction; `harness/status.toml` back to `idle` | docs | PR-16 | no | `make preflight`; fresh-clone reproduction recorded |
