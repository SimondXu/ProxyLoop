...
- R-17 a channel ingest that exhausts on the delivery activity is not re-driven on redelivery (pre-existing; found in the R-1 design, `fix-r1-retryable-update-continues-as-new.md`). **Implemented on `fix/r17-r5-channel-redrive` (PR-3), DB lane pending**: a duplicate whose outbox is in `REDRIVABLE_OUTBOX_STATES` re-sends the identical ingest request (same Update ID), and the Workflow re-runs the delivery activity once R-1 has rolled the run; no `workflow.py` change. Residual invariant: a re-driven `pending` outbox sends without a lookup, so at-most-once delivery relies on the adapter's `send` being idempotent by `idempotency_key`; any real adapter must guarantee this (or look up first). Residual risk: a schedule-to-close `TIMEOUT` lets a retry or re-drive run while the timed-out attempt is still in flight.
