...
| `feat/pr10-agent-status-bar` | PR-10, Stage 4 Agent Status Bar (spec `harness/context/pr10-agent-status-bar-preflight.md`) | Implemented from `main` @ `c018390`: pure `renderStatusBlock(payload)` in the new `apps/web/lib/status-block.ts` is the only builder of the Status Bar text (doing-now line, phase, goal, constraints, current offer, approval + expiry, execution, completion/receipt); rendered as the first context-rail section of `conversation-workspace.tsx`. No API or projection change; `fast` never read; channel delivery status and the Fast gate verdict not shown (not projected). I12 pinned by `tests/contract/test_status_block_boundary.py` (gate imports stdlib + contracts only; no status-block renderer named under `agent_core`, `openai_adapter`, `ml/`) and a Web importer test. 20 new vitest cases red on `main`, green on the branch (log: `harness/log/feat-pr10-agent-status-bar.md`) | independent review; Browser check (needs the DB lane); PR; CI; merge before PR-12 touches `conversation-workspace.tsx` |
