...
| `feat/pr9b-local-fast-gateway` | PR-9b, first half (ml-only; frozen spec `harness/context/pr9-local-distilled-fast-design.md`) | Stdlib PEFT → MLX converter with the committed hash attestation (`ml/serving/`); silent-load guard (252 LoRA layers, all non-zero `lora_b`; a real renamed-tensor load refused); loopback gateway (`proxyloop_evaluation/local_fast/`, interim ml-side wire until 9a's `agent_core/local_fast_wire.py`); M1 parity run and committed report with `phase03c-local-parity-check` in `make test`: **stack parity held** (distilled act agreement 0.983, cloud concordance 1.000; distilled generation p50 21.3 s, 134/240 above 20 s; the root set PR-9a's default timeout to 25 s). Evidence: `harness/log/feat-pr9b-local-fast-gateway.md` | Review: Request Changes, no Blocking; I1 and M1–M7 applied (`harness/code_review/feat-pr9b-local-fast-gateway.md`); the M1 run's code state is recorded and a byte-identical replay of sampled rows from the committed code backs it (no rerun). Merges after 9a, once the gateway uses 9a's wire module. Pending after 9a: M2, gate pass rate, local split reports, wire golden fixtures, the switch to the 9a wire module |
