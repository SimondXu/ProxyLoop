...
| `fix/gate-honesty-r15-g1` | PR-1, R-15 + G-1 strong form | Preflight green, awaiting review. Merged with `main` @ `c73f6a7`; `make preflight` exit 0 (pin still 51); R-15 test 200/200 fresh-process passes after the merge. R-15: the `<= 20` bound was wrong (true invariant 10 ≤ calls ≤ 21; 21 is reachable with one worker) — barrier-based deterministic test, 200/200 passes. G-1: `scripts/check_gated_skips.py` pins 51 gated skips and names the real-dependency gates at the end of `make preflight` | `reviewer`; open question for the reviewer/root: the pin is not enforced when any `PROXYLOOP_TEST_*` is set (alternative: require 0 gated skips when both are set); PR; CI; merge |
