...
| `fix/b2-8-threadpool-runtime-calls` | PR-6, B2-8 | Implemented: `app.py` runs the direct-mode `apply_command`, `current_result`, the readiness probe, and the mailbox route's storage calls via `run_in_threadpool`; red/green test `tests/integration/test_api_event_loop.py`; `make lint`, `typecheck`, `test`, `preflight` green | DB lane (`postgres-check` → `phase05a-check` → `phase06b1-check`); root decision on the direct-mode clock-guard check-then-act (log, "Residual"); `reviewer`; PR; CI; merge. Log `fix-b2-8-threadpool-runtime-calls.md` |
