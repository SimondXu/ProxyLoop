...
| `fix/gate-honesty-r15-g1` | PR-1, R-15 + G-1 strong form | Review: Approve (`harness/code_review/fix-gate-honesty-r15-g1.md`); root decisions applied (enforcement decided by the two variables only, per-file pin); `make preflight` green at 51. Merged with `main` @ `c73f6a7`; R-15 test 200/200 fresh-process passes. R-15: the `<= 20` bound was wrong (true invariant 10 ≤ calls ≤ 21; 21 is reachable with one worker) — barrier-based deterministic test, 200/200 passes. G-1: `scripts/check_gated_skips.py` pins 51 gated skips and names the real-dependency gates at the end of `make preflight` | merge after PR-4 and PR-2 (merge order PR-4 → PR-2 → PR-1); PR-2 adds 2 gated tests, so merge `main` then and update the per-file pin; PR; CI; merge |
