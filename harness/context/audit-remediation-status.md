...
|---|---|---|---|
| `fix/r18-callback-evidence-pairing` | PR-4, R-18 | Review applied (reviewer Approve, Minors 1-3 fixed); merged with `main` @ `c73f6a7`; `make lint`, `make typecheck`, `make test`, `make preflight` green; DB gates pending on merged main | `postgres-check` → `phase05a-check` → `phase06b1-check` on the merged branch; PR; CI; squash merge. Accepted limits are in its log (a consistent forged event+Evidence pair, unchecked `source_ref`/`content_hash`; binding to real deliveries needs a storage change) |
