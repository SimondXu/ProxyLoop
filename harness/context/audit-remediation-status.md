...
| `fix/r12-model-trace-log` | PR-7, R-12 + R-13b | Implemented per the frozen spec `harness/context/r12-model-trace-log-design.md` (append-only model-trace log, `storage_version` 3, bootstrap backfill of version 2 rows under an advisory lock, `_advance` as the single coordinator call with a source guard); non-DB checks green; red evidence in `harness/log/fix-r12-model-trace-log.md` | merge `origin/main` after PR-4; then, with the DB, `postgres-check` → `phase05a-check` → `phase06b1-check`; `reviewer` (storage-format and concurrency change); PR; CI; merge. Independent review: Approve (M1–M6 applied, M7 deferred to PR-8; `harness/code_review/fix-r12-model-trace-log.md`). It adds 6 DB-gated test items, so the PR-1 gated-skip pin moves by +6 once both land |
