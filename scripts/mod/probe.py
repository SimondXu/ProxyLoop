"""Serving probe (ADR-0002): raw measurements of the deployed vLLM endpoint, run from the Mac.
    ...
    python -m scripts.mod.probe --wait-healthy --out docs/decisions/data/vllm-coldstart.json

Endpoint: PROXYLOOP_VLLM_BASE_URL, else looked up from the Modal app; key: PROXYLOOP_VLLM_API_KEY.
...
Exit status is non-zero when any check fails; the JSON is written either way.
"""
...
def latency_messages(index: int, n_notes: int) -> list[dict]:
    """Synthetic latency prompt: a shared n_notes-line prefix and a unique last line."""
    notes = "\n".join(f"Note {i}: the customer asked about invoice {1000 + i} and the monthly "
                      "service charge." for i in range(n_notes))
    return [{"role": "system", "content": "You are a concise assistant. Answer in two sentences."},
            {"role": "user", "content": f"{notes}\n\nRequest {index}: summarise the notes above."}]


def new_record(model: str, phase: str, index: int) -> dict:
    return {"client_request_id": f"pl-probe-{uuid.uuid4().hex[:16]}", "model": model, "phase": phase,
            "prompt_index": index, "server_id": None, "echoed_models": [], "status": None,
            "ttft_s": None, "ttfs_s": None, "total_s": None, "finished": False, "prompt_tokens": None,
            "completion_tokens": None, "text": "", "error": None}


def _finish(record: dict, elapsed: float) -> None:
...
def absorb(record: dict, line: str, elapsed: float) -> None:
    ...
    if chunk.get("model") != record["model"] and record["error"] is None:
        record["error"] = f"echoed model {chunk.get('model')!r} != requested {record['model']!r}"
...
def summarise(records: list[dict]) -> dict:
    """Counts and p50/p95 (linear, numpy's default). A request without a sentence end fails TTFS."""
...
async def stream_one(client: httpx.AsyncClient, model: str, ids: list[int], phase: str, index: int,
                     max_tokens: int) -> dict:
...
async def latency(url: str, headers: dict, prompts: list[list[int]], models: list[str], n: int,
                  max_tokens: int) -> dict:
    """Per model: 2 warm-ups, then n requests at concurrency 1 and n at 4. Every request gets its
    own prompt index, so only the shared prefix is ever cacheable."""
    out, queue = {}, iter(enumerate(prompts))
    async with httpx.AsyncClient(base_url=url, headers=headers, timeout=300) as client:
        for model in models:
            warm = [await stream_one(client, model, ids, "warmup", k, max_tokens)
                    for k, ids in (next(queue) for _ in range(2))]
            out[f"{model}@warmup"] = {"summary": summarise(warm), "requests": warm}
            for concurrency in (1, 4):
                gate, block = asyncio.Semaphore(concurrency), [next(queue) for _ in range(n)]

                async def one(k: int, ids: list[int], model: str = model, gate=gate) -> dict:
                    async with gate:
                        return await stream_one(client, model, ids, "measured", k, max_tokens)
...
def wait_healthy(url: str, timeout_s: float) -> dict:
    """Timer starts after `modal deploy` returned; includes container start, first-time model
    download and hashing when the volume is cold, and vLLM start-up."""
...
def derived(lat: dict, baseline: dict | None) -> dict:
    """Rows the ADR copies: zero-LoRA minus base, and (with --baseline) this run minus the baseline."""
    def delta(a: dict, b: dict, metric: str) -> float | None:
        x, y = a["summary"][metric], b["summary"][metric]
        return None if x is None or y is None else x - y

    metrics = ("ttft_p50_s", "ttfs_p50_s")
    out = {"lora_overhead": {f"c{c}.{m}": delta(lat[f"{config.ZERO_LORA_NAME}@c{c}"],
                                                 lat[f"{config.SERVED_NAME}@c{c}"], m)
                             for c in (1, 4) for m in metrics}}
    if baseline:
        out["minus_baseline"] = {f"{key}.{m}": delta(block, baseline["latency"][key], m)
                                 for key, block in lat.items() if not key.endswith("@warmup")
                                 for m in metrics}
    return out
...
def measure(url: str, key: str, args: argparse.Namespace) -> dict:
    ...
    with httpx.Client(base_url=url, headers=headers, timeout=300) as client:
        ...
        report["liveness"] = {"zero": liveness.compare(client, pairs, config.ZERO_LORA_NAME),
                              "live": liveness.compare(client, pairs, config.LIVE_LORA_NAME)}
    n_notes = 1
    while len(config.chat_ids(tokenizer, latency_messages(0, n_notes))) < args.prompt_tokens:
        n_notes += 1
    models = [config.SERVED_NAME, config.ZERO_LORA_NAME]
    prompts = [config.chat_ids(tokenizer, latency_messages(k, n_notes))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--baseline", help="probe JSON of the pinned variant, for derived deltas")
